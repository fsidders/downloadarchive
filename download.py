...
def joinanddelete(localfile,namevideo):
    try:
        cp = subprocess.run(
            ["ffmpeg", "-f","concat","-safe","0","-i", "{}.txt".format(localfile) ,"-c","copy",namevideo],
            capture_output=True,
            check=True,
            text=True,
        )
...
parser.add_argument("url", help="The URL where the program is stored on archive.org", type=str)
